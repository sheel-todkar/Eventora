...
export default function AdminEvents() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      // fix: handle both plain array and { events: [...] } shape
    ...
    <div className="page container">
      <div className="section-header">
        <div>
          ...
          <p className="section-sub">
            {events.length} event{events.length !== 1 ? 's' : ''} in total
          </p>
        </div>
      ...
      {error && (
        <div className="auth-error" style={{ marginBottom: 20 }}>
          {error}
          <button className="btn btn-secondary btn-sm" style={{ marginLeft: 12 }} onClick={fetchEvents}>
            Retry
          </button>
        </div>
      )}
            ...
            <tbody>
              {events.map(ev => (
                <tr key={ev._id}>
                  ...
                  <td>{ev.availableSeats ?? 0} / {ev.totalSeats ?? 0}</td>
                  <td>
        ...
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) closeModal(); }}>
          <div className="modal">
            ...
            <p className="modal-sub">
              {editId ? 'Update the event details below' : 'Fill in the details for your new event'}
            </p>

            {formError && (
              <div className="auth-error" style={{ marginBottom: 14 }}>{formError}</div>
            )}
