  ...
  return (
    <div className="page">
      ...
      <section className="hero container">
        ...
        <h1>Discover &amp; Book<br /><span>Amazing Events</span></h1>
        <p className="hero-sub">Browse conferences, meetups, workshops and concerts. Book your spot with secure payments.</p>
      </section>
              ...
              <Link to={`/event/${event._id}`} className="event-card" key={event._id}>
                ...
                <div className="event-card-body">
                  ...
                  <div className="event-card-meta">
                    <div className="event-card-meta-item"><CalendarIcon />{formatDate(event.date)}</div>
                    <div className="event-card-meta-item"><MapPinIcon />{event.location}</div>
                  </div>
                  <div className="event-card-footer">
                    <span className="event-price">
                      {event.ticketPrice > 0 ? `₹${event.ticketPrice}` : 'Free'}
                    </span>
                    <span className="event-seats">{event.availableSeats} / {event.totalSeats} seats</span>
                  </div>
                </div>
              </Link>
            ))}
